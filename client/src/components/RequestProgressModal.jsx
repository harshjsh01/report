...
export default function RequestProgressModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg bg-[#0f172a] border border-slate-800 rounded-3xl p-6 shadow-2xl">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-800 mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Request / Inspect Progress</h2>
              <p className="text-xs text-slate-400">Check any developer's real-time GitHub completion status</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-4 animate-in fade-in">
            <div className="p-4 rounded-2xl bg-slate-900 border border-emerald-500/30">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-bold text-white truncate">{result.user.name || result.user.username}</h3>
                  <p className="text-xs text-emerald-400 font-mono">@{result.user.username}</p>
                  {result.user.bio && <p className="text-xs text-slate-400 line-clamp-1 mt-0.5">{result.user.bio}</p>}
                </div>
                <div className="text-right">
                  <div className="text-xl font-black text-emerald-400">{result.user.completionPercentage}%</div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-3 border-t border-slate-800 text-center">
                <div className="p-2 rounded-xl bg-slate-800/60">
                  <div className="text-sm font-bold text-white">{result.user.totalRepos}</div>
                  <div className="text-[10px] text-slate-400">Total Repos</div>
                </div>
                <div className="p-2 rounded-xl bg-amber-500/10 border border-amber-500/20">
                  <div className="text-sm font-bold text-amber-300">{result.user.v1Complete}</div>
                  <div className="text-[10px] text-amber-400">v1.0 Shipped</div>
                </div>
                <div className="p-2 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                  <div className="text-sm font-bold text-emerald-300">{result.user.completed}</div>
            ...
            <div className="flex items-center gap-2 pt-2">
              <button
                ...
                className="flex-1 py-2.5 px-4 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-xl text-xs flex items-center justify-center gap-2 transition-all shadow-lg shadow-emerald-500/20"
              >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="e.g. octocat, torvalds, harshjsh02..."
                  value={targetUsername}
                  onChange={(e) => setTargetUsername(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-700/80 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                  required
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Harsh, Tech Lead, Team Peer..."
                value={requesterName}
                onChange={(e) => setRequesterName(e.target.value)}
                className="w-full bg-slate-900 border border-slate-700/80 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-900 border border-slate-700/80 rounded-xl px-4 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 resize-none"
              />
            </div>
