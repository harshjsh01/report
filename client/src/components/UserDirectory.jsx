...
export default function UserDirectory({ 
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-[#0f172a] to-slate-900 border border-slate-800 p-6 sm:p-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={isLoading}
              className="p-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-2xl border border-slate-700 transition-all disabled:opacity-50"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-6 mt-6 border-t border-slate-800/80">
          <div className="p-3 rounded-2xl bg-slate-900/60 border border-slate-800/80">
            ...
            <div className="text-xs text-slate-400">Tracked Developers</div>
          </div>
          <div className="p-3 rounded-2xl bg-slate-900/60 border border-slate-800/80">
            <div className="text-xl sm:text-2xl font-black text-emerald-400">{avgCompletion}%</div>
            <div className="text-xs text-slate-400">Avg Portfolio Completion</div>
          </div>
          <div className="col-span-2 sm:col-span-1 p-3 rounded-2xl bg-slate-900/60 border border-slate-800/80">
            <div className="text-xl sm:text-2xl font-black text-teal-300">{requests.length}</div>
            <div className="text-xs text-slate-400">Peer Checks Conducted</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-[#0e1424] p-3 rounded-2xl border border-slate-800">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-slate-900 border border-slate-700/70 rounded-xl pl-9 pr-4 py-2 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500"
          />
        </div>
        <div className="text-xs text-slate-400 px-2 flex items-center justify-end">
          Showing <span className="font-bold text-white mx-1">{filteredUsers.length}</span> of {users.length} developers
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredUsers.map(user => {
            ...
            return (
              <div 
                key={user.username}
                className={`flex flex-col justify-between p-5 rounded-3xl bg-slate-900/60 border ${
                  isSelf ? 'border-emerald-500/40 shadow-lg shadow-emerald-500/5' : 'border-slate-800'
              ...
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3">
                      <img 
                        ...
                        alt={user.username}
                        className="w-12 h-12 rounded-2xl bg-slate-800 border border-slate-700/80 object-cover"
                      />
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <a 
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-xs text-slate-400 hover:text-emerald-400 font-mono flex items-center gap-1"
                          >
                  ...
                  {/* KPI Badges */}
                  <div className="grid grid-cols-3 gap-2 p-2.5 rounded-2xl bg-slate-950/40 border border-slate-800/80 text-center mb-4">
                    <div>
                      <div className="text-sm font-extrabold text-white">{user.totalRepos || 0}</div>
                      <div className="text-[10px] text-slate-400">Repos</div>
                    </div>
                    <div>
                      <div className="text-sm font-extrabold text-amber-300">{user.v1Complete || 0}</div>
                      <div className="text-[10px] text-amber-400">v1.0 Shipped</div>
                    </div>
                    <div>
                      <div className="text-sm font-extrabold text-emerald-300">{user.completed || 0}</div>
                  ...
                  {user.topLanguages && user.topLanguages.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {user.topLanguages.map(lang => (
                        <span 
                          key={lang} 
                          className="px-2 py-0.5 rounded-lg bg-slate-800 text-[10px] font-semibold text-slate-300 border border-slate-700/60"
                        >
                          {lang}
                        </span>
                      ))}
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-slate-800 hover:bg-emerald-500 hover:text-slate-950 text-slate-200 text-xs font-bold flex items-center justify-center gap-2 transition-all group-hover:border-emerald-500/50 cursor-pointer"
                >
        ...
        <div className="rounded-3xl bg-slate-900/40 border border-slate-800 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white">Recent Progress Check Activity</h3>
            </div>
            <span className="text-xs text-slate-400">{requests.length} checks logged</span>
          </div>
              ...
              <div key={req.id} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs text-slate-300">
                    <span className="font-semibold text-white">{req.requesterName}</span> requested progress check for{' '}
                    <button
