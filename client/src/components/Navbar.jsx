...
export default function Navbar({ 
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-slate-800 bg-[#0d131f]/80 backdrop-blur-md">
      
      {/* Viewing Peer Notice Banner */}
      {viewingUser && (
        <div className="bg-emerald-500/10 border-b border-emerald-500/20 px-4 py-2 text-xs flex items-center justify-between text-emerald-300">
          <div className="flex items-center gap-2 max-w-7xl mx-auto w-full">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            <span>
              Inspecting portfolio for <strong className="font-mono text-white">@{viewingUser}</strong>
            </span>
            <button
              onClick={onClearViewingUser}
              className="ml-auto inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-500 hover:bg-emerald-400 text-slate-950 rounded-lg font-bold text-[11px] transition-all cursor-pointer"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-3.5 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center justify-between w-full md:w-auto gap-4">
          ...
          {/* Primary View Tabs */}
          <div className="hidden sm:flex items-center bg-slate-900 border border-slate-800 rounded-2xl p-1 text-xs">
            <button
              onClick={() => {
        ...
        <div className="flex items-center gap-2.5 w-full md:w-auto justify-end flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border border-emerald-500/30 text-emerald-300 hover:text-white hover:border-emerald-500/60 text-xs font-bold transition-all cursor-pointer shadow-sm"
          >
